...
videos = glob.glob(os.path.join(root_dir, "cam*.mp4"))
...
for filename in os.listdir(root_dir):
        # 检查是否为所需的文件格式且不是.mp4文件
        if filename.startswith("cam") and not filename.endswith(".mp4"):
            filepath=os.path.join(root_dir,filename)
            selected_files.append(filepath)
videos = sorted(selected_files)
image_paths = []
