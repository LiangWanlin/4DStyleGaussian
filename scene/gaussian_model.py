...
from scene.projector.feature_mlp import FeatureMLP
from scene.projector.color_mlp import ColorMLP
...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, args):
        ...
        self.feat_dim = 32  
        self.color_mlp = ColorMLP(self.feat_dim, 3)  
        self.feature_mlp = FeatureMLP(self.feat_dim, 32)
    def capture(self):
        return (
            ...
            self.color_mlp.state_dict(),
            self.feature_mlp.state_dict()
        )
    ...
    def restore(self, model_args, training_args):
        ...
        self.spatial_lr_scale,
        color_mlp_dict,
        feature_mlp_dict) = model_args
        self._deformation.load_state_dict(deform_state)
        self.color_mlp.load_state_dict(color_mlp_dict)
        self.feature_mlp.load_state_dict(feature_mlp_dict)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, time_line: int):
        ...
        # features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda()  # N_points x 3 x 16
        # features[:, :3, 0 ] = fused_color
        # features[:, 3:, 1:] = 0.0
        features = torch.zeros((fused_color.shape[0], self.feat_dim, (self.max_sh_degree + 1) ** 2 )).float().cuda()  # N_points x 32 x 16
        ...
        self.color_mlp = self.color_mlp.to("cuda")
        self.feature_mlp = self.feature_mlp.to("cuda")
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {'params': self.color_mlp.parameters(), 'lr': training_args.feature_lr, "name": "color_mlp"},
            {'params': self.feature_mlp.parameters(), 'lr': training_args.feature_lr, "name": "feature_mlp"}
    ...
    def load_model(self, path):
        ...
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
        weight_dict_mlp = torch.load(os.path.join(path, "color_mlp.pth"), map_location="cuda")
        self.color_mlp.load_state_dict(weight_dict_mlp)
        self.color_mlp = self.color_mlp.to("cuda")
        weight_dict_feature_mlp = torch.load(os.path.join(path, "feature_mlp.pth"), map_location="cuda")
        self.feature_mlp.load_state_dict(weight_dict_feature_mlp)
        self.feature_mlp = self.feature_mlp.to("cuda")
    ...
    def save_mlp(self, path):
        torch.save(self.color_mlp.state_dict(), os.path.join(path, "color_mlp.pth"))
        torch.save(self.feature_mlp.state_dict(), os.path.join(path, "feature_mlp.pth"))
    ...
    def load_ply(self, path):
        ...
        features_dc = np.zeros((xyz.shape[0], self.feat_dim, 1))
        for idx, attr_name in enumerate(dc_f_names):
            # features_extra[:, idx] = np.asarray(plydata.elements[0][attr_name])
            features_dc[:, idx, 0] = np.asarray(plydata.elements[0][attr_name])

        extra_f_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("f_rest_")]
        extra_f_names = sorted(extra_f_names, key = lambda x: int(x.split('_')[-1]))
        # assert len(extra_f_names)==3*(self.max_sh_degree + 1) ** 2 - 3
        assert len(extra_f_names)==self.feat_dim*((self.max_sh_degree + 1) ** 2 - 1)
        features_extra = np.zeros((xyz.shape[0], len(extra_f_names)))
        for idx, attr_name in enumerate(extra_f_names):
            features_extra[:, idx] = np.asarray(plydata.elements[0][attr_name])
        # Reshape (P,F*SH_coeffs) to (P, F, SH_coeffs except DC)
        features_extra = features_extra.reshape((features_extra.shape[0], self.feat_dim, (self.max_sh_degree + 1) ** 2 - 1))
    ...
    def standard_constaint(self):
        ...
        means3D_deform, scales_deform, rotations_deform, _ ,_= self._deformation(means3D, scales, rotations, opacity,shs, time)
