...
class Affinity_Propagate(nn.Module):
    ...
    def forward(self, guidance, blur_img, sparse_img=None):
    ...
    def affinity_normalization(self, guidance):
        ...
        gate_wb = torch.div(gate_wb, abs_weight + 0.0001)
        gate_wb = torch.nan_to_num(gate_wb)
    ...
    def pad_blur_img(self, blur_img):
        # top pad
        left_top_pad = nn.ZeroPad2d((0,2,0,2))
        blur_img_1 = left_top_pad(blur_img).unsqueeze(1)
        center_top_pad = nn.ZeroPad2d((1,1,0,2))
        blur_img_2 = center_top_pad(blur_img).unsqueeze(1)
        right_top_pad = nn.ZeroPad2d((2,0,0,2))
        blur_img_3 = right_top_pad(blur_img).unsqueeze(1)

        # center pad
        left_center_pad = nn.ZeroPad2d((0,2,1,1))
        blur_img_4 = left_center_pad(blur_img).unsqueeze(1)
        right_center_pad = nn.ZeroPad2d((2,0,1,1))
        blur_img_5 = right_center_pad(blur_img).unsqueeze(1)

        # bottom pad
        left_bottom_pad = nn.ZeroPad2d((0,2,2,0))
        blur_img_6 = left_bottom_pad(blur_img).unsqueeze(1)
        center_bottom_pad = nn.ZeroPad2d((1,1,2,0))
        blur_img_7 = center_bottom_pad(blur_img).unsqueeze(1)
        right_bottm_pad = nn.ZeroPad2d((2,0,2,0))
        blur_img_8 = right_bottm_pad(blur_img).unsqueeze(1)

        result_img = torch.cat((blur_img_1, blur_img_2, blur_img_3, blur_img_4,
                                  blur_img_5, blur_img_6, blur_img_7, blur_img_8), 1)
        return result_img
    ...
    def normalize_gate(self, guidance):
        ...
        gate1_x1_g1_cmb = torch.div(gate1_x1_g1, elesum_gate1_x1 + 0.0001)
        gate1_x1_g1_cmb = torch.nan_to_num(gate1_x1_g1_cmb)
        gate1_x1_g2_cmb = torch.div(gate1_x1_g2, elesum_gate1_x1 + 0.0001)
        gate1_x1_g2_cmb = torch.nan_to_num(gate1_x1_g2_cmb)
